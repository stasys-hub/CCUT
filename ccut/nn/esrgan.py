...
class GeneratorMM(nn.Module):
    def __init__(self, in_channels=1, num_channels=64, num_blocks=23):
        ...
        #     UpsampleBlock(num_channels), UpsampleBlock(num_channels),
        # )
        self.rrdb_final = nn.Sequential(
            nn.Conv2d(num_channels, num_channels, 3, 1, 1, bias=True),
            nn.LeakyReLU(0.2, inplace=True),
            nn.Conv2d(num_channels, in_channels, 3, 1, 1, bias=True),
        )
        self.mha = MultiHeadAttention(128, 128, 4)
        self.hic_attention = HiCAttention(128, 128, 128)

    def forward(self, x_1, x_2):
        initial = self.initial(x_1)
        x_1 = self.conv(self.residuals(initial)) + initial
        # x = self.upsamples(x)
        x_1 = self.rrdb_final(x_1)
        x_2 = self.mha(x_2)
        out = self.hic_attention(x_1, x_2)
        return out
