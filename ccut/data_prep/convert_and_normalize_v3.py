...
def clamp_and_norm(matrix, cutoff=None) -> np.ndarray:
    ...
    if args.normalize:
        ...
        if matrix_capped.max() > 0:  # Avoid division by zero
            matrix_normalized = (matrix_capped - matrix_capped.min()) / (
                matrix_capped.max() - matrix_capped.min()
            )
        else:
...
def parse_chromosomes_arg(chromosomes_str, available_chromosomes):
    ...
    if "-" in chromosomes_str:
        ...
        selected_chromosomes = [
            ...
            if f"chr{i}" in available_chromosomes
        ]
    else:
        selected_chromosomes = (
            [f"chr{chromosomes_str}"]
            if f"chr{chromosomes_str}" in available_chromosomes
            else []
        )
...
def main(
    ...
    print(
        f"Processing cooler file: {cooler_path} for chromosomes: {selected_chromosomes} with percentile cutoff: {percentile} and direct cutoff: {cutoff}"
    )

    # Setup data for multiprocessing
    task_data = [
        (clr, chrom, output_path, prefix, cutoff) for chrom in selected_chromosomes
    ]
    ...
    with Pool(processes=processes) as pool:
        results = list(
            tqdm(
                pool.imap(process_chromosome, task_data),
                total=len(task_data),
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Process cooler files and save processed data."
    )
    parser.add_argument("cooler_path", type=str, help="Path to the cooler file.")
    parser.add_argument(
        "--output_path", type=str, default=".", help="Directory to save output files."
    )
    parser.add_argument(
        "--prefix", type=str, default="", help="File prefix for the output."
    )
    parser.add_argument(
        "--chromosomes",
        type=str,
        required=True,
        help="Chromosome or range of chromosomes to process, e.g., '19' or '19-22'",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        type=float,
        default=99.9,
        help="Percentile cutoff for outlier removal.",
    )
    parser.add_argument(
        "--cutoff",
        type=float,
        default=None,
        help="Direct cutoff value for capping matrix values.",
    )
    parser.add_argument("--balance", action="store_true", help="Use balanced data")
    parser.add_argument(
        "--normalize", action="store_true", help="Min-max nomraliztion between 0-1"
    )
