...
def generate_sliding_window_coords(
    ...
    if threshold > 0:
        chrom_size = max(
            0, chrom_size - threshold
        )  # Adjust chrom size based on threshold

    step_size = int(window_size * resolution * step)
    exact_window_size = (
        window_size * resolution
    )  # This is the exact size each window should be
...
def main(args):
    ...
    all_coords = []
    for chrom in chroms:
        coords = generate_sliding_window_coords(
            clr,
            chrom,
            args.window_size,
            args.resolution,
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Generate sliding window coordinates for cooler files with distance filter and step parameter"
    )
    parser.add_argument(
        "--cooler_file", type=str, required=True, help="Path to the cooler file"
    )
    parser.add_argument(
        "--chromosomes",
        type=str,
    ...
    parser.add_argument(
        "--resolution", type=int, default=10000, help="Resolution of the cooler file"
    )
    parser.add_argument(
        "--max_distance",
        type=int,
        default=2000000,
        help="Maximum distance between windows for inclusion",
    )
    parser.add_argument(
        "--threshold",
        type=int,
        default=0,
        help="Substracts treshold from maximum chrom size",
    )
    parser.add_argument(
        "--step",
        type=float,
        default=1.0,
        help="Step size as a fraction of window size for overlapping windows",
    )
    parser.add_argument(
        "--output_path",
        type=str,
        default="coordinates.csv",
        help="Path to save the coordinates CSV",
    )
